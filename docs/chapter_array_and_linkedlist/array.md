...
<u>数组（array）</u>是一种线性数据结构，其将*相同类型的元素*存储在连续的内存空间中。我们将元素在数组中的位置称为该元素的<u>索引（index）</u>。下图展示了数组的主要概念和存储方式。
...
值得注意的是，由于数组的长度是固定的，因此插入一个元素必定会导致<u>数组尾部元素“丢失”</u>。我们将这个问题的解决方案留在“列表”章节中讨论。
...
请注意，删除元素完成后，原先末尾的元素变得“无意义”了，所以我们*无须特意去修改它*。
