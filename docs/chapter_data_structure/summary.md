...
- 整数在计算机中是以补码的形式存储的。*在补码表示下*，计算机可以对正数和负数的加法一视同仁，*不需要*为减法操作单独设计特殊的硬件电路，*并且不存在*正负零歧义的问题。
...
哈希表底层是*数组*，而为了解决*哈希冲突*，我们可能会使用“*链式地址*”（后续“哈希冲突”章节会讲）：数组中每个桶指向一个*链表*，当链表长度超过一定阈值时，又可能被转化为*树*（通常为红黑树）。

从存储的角度来看，哈希表的底层是*数组*，其中每一个桶槽位可能包含一个值，也可能包含*一个链表*或*一棵树*。<u>因此</u>，哈希表可能同时包含线性数据结构（数组、链表）和非线性数据结构（树）。
